...
for M in M_values:
    ...
    parameter_vector_1 = np.linalg.solve(XTX, mat_trans @ target_matrix[:, 0])
    parameter_vector_2 = np.linalg.solve(XTX, mat_trans @ target_matrix[:, 1])
    parameter_vector_3 = np.linalg.solve(XTX, mat_trans @ target_matrix[:, 2])
    ...
    for i in range(0, 3):
        ...
        axes[i].set_xlabel("Actual Values (t%d)" % (i + 1))
        axes[i].set_ylabel("Predicted Values")
        axes[i].legend()
        axes[i].set_title("Degree=%d, No Reg" % M)
    axes[-1].axis('off')
